<?php 
    function view_commentPost($post, $err=null) {
        ob_start();
        $title = "Commenter un post";
...
<?php if(str_contains($post['image_link'], "./view")) {?>
    ...
    <form action="/index.php?action=commentPost&id=<?= $post['id'] ?>" method="POST" enctype="multipart/form-data">
        <!-- Text -->
        <div class="grid grid-cols-2 grid-flow-row mt-5">
            <p class="col-span-1 text-lg text-black">Commentaire</p>
            <textarea name="formCommentPostComment" class="textarea"></textarea>
        </div>
        <!-- divider 2 -->
        ...
        <div class="flex flex-col w-full lg:flex-row mt-10">
            <div class="grid flex-grow h-32">
                <div class="mb-10">
                    ...
                    <input name="formCommentPostFile" type="file" accept=".png,.jpg,.jpeg" class="file-input file-input-sm w-full mt-2" />
                </div>
            </div> 
            ...
            <div class="grid flex-grow h-32">
                <div>
                    ...
                    <label class="input-group mt-2">
                        <span>Lien</span>
                        <input name="formCommentPostLink" type="text" class="input input-sm input-bordered w-full" />
                    </label>
                </div>
            </div>
        </div>
